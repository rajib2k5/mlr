...
test_that("selectFeatures", {
  ...
  fr = selectFeatures(lrn, task = mytask, resampling = rdesc, control = ctrl, show.info = FALSE)
  ...
  fr = selectFeatures(lrn, task = multiclass.task, resampling = rdesc, bit.names = bns, bits.to.features = btf, control = ctrl, show.info = FALSE)
