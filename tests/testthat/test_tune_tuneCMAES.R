...
test_that("tuneCMAES with budget", {
  ...
  ctrl = makeTuneControlCMAES(start = list(cp = 0.05, minsplit = 5L), maxit = 5, budget = 20)
  expect_error(tuneParams(makeLearner("classif.rpart"), multiclass.task, res,
  ...
  expect_identical(length((ctrl1$extra.args$lambda + 1) * ctrl1$extra.args$maxit), 0L)
  expect_identical(length(ctrl1$extra.args$lambda * (ctrl1$extra.args$maxit + 1)), 0L)
  tr1 = tuneParams(makeLearner("classif.rpart"), multiclass.task, res,
    par.set = ps1, control = ctrl1)
